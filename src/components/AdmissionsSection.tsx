    ...
    <section id="admissions" ref={ref} className="section-padding bg-secondary/30">
      <div className="container mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-5 sm:gap-6 md:gap-8 max-w-4xl mx-auto">
          ...
          >
            <h3 className="font-bold text-base sm:text-lg mb-3 sm:mb-4">Batch Timings</h3>
            <div className="space-y-2 sm:space-y-3">
              {batches.map((b) => (
                <div key={b.label} className="flex flex-col sm:flex-row sm:justify-between sm:items-center py-2 border-b border-border/50 last:border-0 gap-1 sm:gap-0">
                  <span className="font-medium text-xs sm:text-sm">{b.label}</span>
                  <span className="text-xs sm:text-sm text-muted-foreground">{b.time}</span>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            className="glass-card p-4 sm:p-5 md:p-6"
          >
            <h3 className="font-bold text-base sm:text-lg mb-3 sm:mb-4">What You Get on Admission</h3>
